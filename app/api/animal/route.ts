...
export async function POST(req: NextRequest) {
  ...
  try {
    const { name, breed, hoursTrained, birthMonth, birthDay, birthYear, note, userEmail } = await req.json();

    // Validate required fields
    if (!name || !breed || !hoursTrained || !birthMonth || !birthDay || !birthYear || !note || !userEmail) {
